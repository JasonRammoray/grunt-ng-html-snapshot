...
(function () {
  ...
  function loadPages(websitePages) {
    ...
    var pageLoadHandlerFired = false;
    ...
    pageObj.onLoadFinished = function (status) {
      ...
      pageLoadHandlerFired = true;

      if (status !== 'success') {
        utils.log.add('Page ' + websitePageUrl + ' loading failed. Proceeding to the next page');

        return loadPages(websitePages);
      }
